...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
 void DriveTrain::motorCountEncoder(double enct, double pwr, Direction dir)
 {
	 int lft_target = leftEnc->GetRaw() + enct;
	 int rgt_target = rightEnc->GetRaw() + enct;
	 ...
	 if (dir == Reverse) {
		 ...
		 rgt_target *= -1;
		 pwr *= -1;
		 while(lft_target  <= leftEnc->GetRaw() + enct) {
			 freeTalon->Set(pwr);
		 }
	 }
...
/*----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
 void DriveTrain::driveByTime(double timeValue, double lPwr, double rPwr, Direction dir)
 {
	 driveTime->Start();
	 driveTime->Reset();
	 if(dir == Direction::Reverse)
